...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        # Use the skew-symmetric part to find the axis * sin(angle)
        axis = np.array([
            R_rel[2, 1] - R_rel[1, 2],
            R_rel[0, 2] - R_rel[2, 0],
            R_rel[1, 0] - R_rel[0, 1]
        ]) / 2  # Normalized by 2 as per the skew-symmetric properties
    ...
    def distance_and_angle(G, H):
        ...
        # Compute the angle using the trace of the relative rotation matrix
        trace_R_rel = np.trace(R_rel)
        cos_angle = (trace_R_rel - 1) / 2
        cos_angle = np.clip(cos_angle, -1.0, 1.0)  # Ensures the argument is in the valid range
    ...
    def is_valid_solution(self,q,target):
        ...
        for i, q_val in enumerate(q):
            if not (lower_limits[i] <= q_val <= upper_limits[i]):
                return False, f"Joint {i+1} is out of limits: {q_val} not in [{lower_limits[i]}, {upper_limits[i]}]"

        # Compute the end effector pose for the candidate solution q
        end_effector_pose,T0e = FK().forward(q)


        distance, angle = IK.distance_and_angle(T0e, target)

        # Check if within tolerance
        if distance > tolerance:
            return False, f"Distance tolerance exceeded: {distance} > {tolerance}"
        if angle > tolerance:
            return False, f"Angle tolerance exceeded: {angle} > {tolerance}"

        return True, "Solution is valid and within tolerance"
    ...
    def end_effector_task(q, target, method):
        # Compute the current end effector pose for the given q
        _, T0e = FK().forward(q)

        # Calculate the translational error
        translation_error = target[:3, 3] - T0e[:3, 3]

        # Calculate the rotational error using quaternions
        R_current = R.from_matrix(T0e[:3, :3])
        R_target = R.from_matrix(target[:3, :3])
        rotation_error_quat = R_target * R_current.inv()
        rotation_error = rotation_error_quat.as_rotvec()

        # Scale position and rotation errors for balanced control
        position_scale = 1.0
        rotation_scale = 0.1
        error = np.hstack((position_scale * translation_error, rotation_scale * rotation_error))

        # Compute the Jacobian for the current configuration q
        J = calcJacobian(q)

        # Choose the method to compute dq
        if method:  # J_pseudo
            J_pseudo_inv = np.linalg.pinv(J, rcond=1e-4)  # Regularized pseudo-inverse
    ...
    ###############################
    ## Inverse Kinematics Joint Positions Solver ##
    ###############################

    def inverse_q(self, start, target):
        """
        Uses gradient descent to move the Panda robot arm from the startng configuration to
        the goal configuration by calculating the corresponding joint angles.

        INPUTS:
        start - 1x7 numpy array representing the starting joint angles for a configuration
        goal - 4x4 transformation matrix of the goal to the world frame
        ...
        q = start.copy()
        q_path = np.append(q_path, np.reshape(q, (1, 7)), axis=0)

        iteration = 0
        while iteration < self.max_steps:
            # Calculate the end-effector velocity to minimize position/orientation error
            joint_positions, current_pose = self.fk.forward(q)
            v, omega = self.displacement_and_axis(target, current_pose)
            dq_ik = self.end_effector_task(q, target, method=True)

            dq_center = self.joint_centering_task(q)


            J = calcJacobian(q)
            J_plus = np.transpose(J) @ np.linalg.inv(J @ np.transpose(J) + 1e-6 * np.eye(J.shape[0]))
            dq = dq_ik + (np.eye(7) - J_plus @ J) @ dq_center
            ...
            # Check convergence
            distance, angle = self.distance_and_angle(current_pose, target)
            if distance <= self.linear_tol and angle <= self.angular_tol:
                return q, q_path, True, "Solution found within tolerance."
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            if success or i >= self.max_steps or np.linalg.norm(dq) < self.min_step_size:
                break
    ...
    def inverse_cal(self, start, target):
        """
        Uses gradient descent to move the Panda robot arm from the startng configuration to
        the goal configuration by calculating the corresponding joint angles.
