...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        axis = np.zeros(3)
        current_displacement = current[:3, 3]
        target_displacement = target[:3, 3]
        current_orientation = current[:3, :3]
        target_orientation = target[:3, :3]
        displacement = target_displacement - current_displacement
        axis = calcAngDiff(target_orientation, current_orientation)
    ...
    def is_valid_solution(self,q,target):
        ...
        jointPositions, T0e = self.fk.forward(q)
        distance, angle = IK.distance_and_angle(target, T0e)

        if not (np.all(self.lower <= q) and np.all(q <= self.upper)):
            message = "Joint limits violated"
        elif distance >= self.linear_tol:
            message = "Position tolerance not met"
        elif angle >= self.angular_tol:
            message = "Orientation tolerance not met"
        else:
            success = True
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            dq_ik = IK.end_effector_task(q,target, method).flatten()
            ...
            if method == 'J_pseudo':
                J_inv = np.linalg.pinv(J)
                null = (I - J_inv @ J) @ dq_center
                dq = dq_ik + null.flatten()
            elif method == 'J_transpose':
                J_inv = J.T
                dq = dq_ik
            ...
            i += 1
            if i == self.max_steps or np.linalg.norm(dq) < self.min_step_size:
                break
...
if __name__ == "__main__":
    ...
    target = np.array([
        [-7.88600633e-01 , 6.14905718e-01, -2.91398040e-06,  4.88820865e-01],
        [ 6.14905718e-01,  7.88600633e-01, -4.29003158e-06 ,-1.17868542e-01],
        [-3.39998163e-07, -5.17494483e-06, -1.00000000e+00,  2.25388309e-01],
        [ 0.00000000e+00 , 0.00000000e+00 , 0.00000000e+00 , 1.00000000e+00],
    ])

    # Using pseudo-inverse 
    q_pseudo, rollout_pseudo, success_pseudo, message_pseudo = ik.inverse(target, seed, method='J_pseudo', alpha=.2)
    ...
    # Using pseudo-inverse 
    # q_trans, rollout_trans, success_trans, message_trans = ik.inverse(target, seed, method='J_transpose', alpha=.5)
